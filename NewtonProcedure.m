function [ xFinal ] = NewtonProcedure( fun, x0, h, tol, maxiter, plt, range)
% Name:         Local Newton-Procedure
...
x(:,counter) = x0;
dx = 10 * ones(size(x0,1),maxiter);

tic
% Loop for the local newton procedure
...
if counter == maxiter
    ...
    disp('Newteon procedure completed!')
    counter
    Norm = norm(dx)
end
